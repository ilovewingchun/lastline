...
out_file_ip = "blacklist_ip.txt" # Required. Save results to this file name.
out_file_domain = "blacklist_domain.txt" # Required. Save results to this file name.
...
for i in range(len(a)): # Iterate over first level list
    ...
    if whitelist_file:
        ...
        list_ip = [x for x in list_ip if x not in wl]
    ...
    list_domain.append(hostname)
    if whitelist_file:
        c = [x for x in c if x not in wl] # Remove those entries that are inside whitelist.
        list_domain = [x for x in list_domain if x not in wl]
        c = list(set(c)) # Retrieve each elements inside list c, using set function to remove duplicate entries and store in c.
...
c = filter(len,c)
list_ip = filter(len,list_ip)
list_domain = filter(len,list_domain)
...
for item in range(len(list_ip)): # Iterate over our list c.
    e_ip = list_ip[item] # Store each elements inside a new variable e
    w = csv.writer(fo, lineterminator="\n") # Using csv function to write each value to a newly definied variable w, which actually writes to previously opened file fo.
...
for item in range(len(list_domain)): # Iterate over our list c.
    e_domain = list_domain[item] # Store each elements inside a new variable e
    w = csv.writer(fo, lineterminator="\n") # Using csv function to write each value to a newly definied variable w, which actually writes to previously opened file fo.
