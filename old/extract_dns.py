...
'''
    ...
    This script uses command line arguments to extract domain name entry from Lastline exported event(default to events.json), make a list and store it to a txt file(default to block_dns.txt).
...
parser.add_argument('-o','--output_file',action="store",default='block_dns.txt',dest='out_file',help='Extracted ip list')
...
json_data = open(in_file) # Open our json file.
...
for i in range(len(a)): # Iterate over first level list
    b = a[i]["entry"] # Iterate retrieve IP value for key "dst_host" inside list a
...
for item in range(len(d)): # Iterate over our list d.
    ...
    w.writerow([e]) # Write each DNS from e to destination file.
